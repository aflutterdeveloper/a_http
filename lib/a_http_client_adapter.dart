...
class AHttpClientAdapter extends DefaultHttpClientAdapter {
  AHttpClientAdapter(this._devEnable, this._proxy);

  HttpClient _defaultHttpClient;

  bool _closed = false;
  final bool _devEnable;
  final String _proxy;
  ...
  Future<ResponseBody> fetch(
    ...
    ALog.info("AHttpClientAdapter", "response reading");
    ...
    ALog.info("AHttpClientAdapter", "response complete");
    return ResponseBody(
      stream,
  ...
  HttpClient _configHttpClient(Future cancelFuture, int connectionTimeout) {
    ...
    if (_devEnable) {
      if (_proxy?.isNotEmpty == true) {
        _defaultHttpClient.findProxy = (uri) {
          return "PROXY $_proxy";
        };
      }

      _defaultHttpClient.badCertificateCallback =
          (X509Certificate cert, String host, int port) {
        ALog.info("AHttp", "bad X509Certificate");
        return true;
      };
    }
